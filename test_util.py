...
from export_utils import export_batch
from edge_diagnostic import log_edge_closure
...
def test_model(model, data_loader, logger, export_dir='./export',
               score_thresh=0.5, edge_thresh=0.5, model_cfg=None):
    dataloader_iter = iter(data_loader)
    os.makedirs(export_dir, exist_ok=True)
    export_counter = 0
    with tqdm.trange(0, len(data_loader), desc='test', dynamic_ncols=True) as tbar:
        model.use_edge = True
        statistics = {'tp_pts': 0, 'num_label_pts': 0, 'num_pred_pts': 0, 'pts_bias': np.zeros(3, float),
        ...
        for cur_it in tbar:
            ...
            # ---- edge-head closure diagnostic (observability only) ----
            if model_cfg is not None:
                log_edge_closure(batch, edge_thresh=edge_thresh, logger=logger)
            # ---- export per-sample CloudCompare artifacts ----
            export_counter = export_batch(
                batch, export_dir, export_counter,
                score_thresh=score_thresh, edge_thresh=edge_thresh,
            )
            # --------------------------------------------------
