...
def main():
    ...
    # Search order prefers the most-trained checkpoint family available:
    #   1. *checkpoint_epoch_*.pth   (legacy joint training, --stage all)
    #   2. checkpoint_epoch_*.pth    (same without leading wildcard)
    #   3. edge_epoch_*.pth          (stage-2 output: full network, recommended)
    #   4. vertex_epoch_*.pth        (stage-1 only -- edge head untrained, exports
    #                                  are useful for inspection but edge metrics
    #                                  will be meaningless)
    ...
    if len(ckpt_list) == 0:
        ckpt_list = glob.glob(str(ckpt_dir / 'checkpoint_epoch_*.pth'))
    if len(ckpt_list) == 0:
        ckpt_list = glob.glob(str(ckpt_dir / 'edge_epoch_*.pth'))
    if len(ckpt_list) == 0:
        ckpt_list = glob.glob(str(ckpt_dir / 'vertex_epoch_*.pth'))
    if len(ckpt_list) == 0:
        ckpt_list = glob.glob(str(cfg.ROOT_DIR / 'checkpoint_epoch_*.pth'))
    assert len(ckpt_list) > 0, (
        'No checkpoint found! Looked in %s and %s for any of: '
        '*checkpoint_epoch_*.pth, edge_epoch_*.pth, vertex_epoch_*.pth.'
        % (str(ckpt_dir), str(cfg.ROOT_DIR))
    )
    ckpt_list.sort(key=os.path.getmtime)
    logger.info('Found %d checkpoint(s); loading newest: %s' % (len(ckpt_list), ckpt_list[-1]))
    model_utils.load_params(net, ckpt_list[-1], logger=logger)
    # --------------------------------------

    logger.info('**********************Start testing**********************')
    logger.info(net)
    test_model(net, test_loader, logger, model_cfg=cfg.MODEL)
