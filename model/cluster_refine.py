...
# tips: change from batch to stack
    def forward(self, batch_dict):
        ...
        # --- XY-projected clustering (optional, config-gated) ----------------
        # DBSCAN groups points by 3D distance; the vertical smear of the
        # offset-shifted blobs inflates pairwise distances and starves clusters
        # (harder to satisfy min_pts), and can split one plan-view corner into
        # two height-separated clusters. Clustering on XY only fixes both.
        #
        # We build a SEPARATE flattened copy for the DBSCAN call. The original
        # 3D `pts_cluster` is still passed to get_cluster_pts, so the centroid
        # coordinates (the actual keypoints) keep their real Z. `offset_pts`
        # (full 3D) still feeds fea_refine_module, so the trained SA module sees
        # the same geometry it was trained on -> no retraining required.
        #
        # Only the Z of VALID (above-threshold) points is zeroed. The rejected
        # points keep their (-10,-10,-10) sentinel so DBSCAN ignores them
        # exactly as before.
        if getattr(self.model_cfg.Cluster, 'project_xy', False):
            pts_cluster_db = pts_cluster.clone()
            valid = pts_score > score_thresh
            pts_cluster_db[..., 2] = torch.where(
                valid,
                torch.zeros_like(pts_cluster_db[..., 2]),
                pts_cluster_db[..., 2],
            )
        else:
            pts_cluster_db = pts_cluster

        cluster_idx = dbscan_cluster(self.model_cfg.Cluster.eps, self.model_cfg.Cluster.min_pts, pts_cluster_db)
        key_pts, num_cluster = get_cluster_pts(pts_cluster, cluster_idx)
        # ---------------------------------------------------------------------
        # Run matcher whenever GT vectors are available. The matcher's outputs
        # are needed by:
        #   - self.training=True (this module's own loss)
        #   - stage-2 edge training (frozen cluster_refine in eval(), but the
        #     edge head still needs `matches` for supervision)
        # Inference doesn't have GT vectors -> falls through to the else branch.
        gt_vecs = batch_dict.get('vectors', None)
        has_gt = gt_vecs is not None and (
            isinstance(gt_vecs, torch.Tensor) and gt_vecs.numel() > 0
        )
        if has_gt:
             new_pts, targets, labels, matches, new_xyz_batch_cnt = self.matcher(key_pts, gt_vecs)
